...
export class DashboardComponent {
  ...
  generatePassword() {
    ...
    if (
      this.useMixed &&
      (this.useUpperCase || this.useLowerCase || this.useNumbers)
    ) {
      this.alert = true;
      this.errorMessage =
        'You cannot select "Mixed" along with other options (Uppercase, Lowercase, Numbers).';
      return;
    } else {
      this.alert = false;
    }
    ...
    if (!characters) {
      this.alert = true;
      this.errorMessage =
        'Please select at least one option for the password generation.';
      return;
    }
